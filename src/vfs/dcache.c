//
// $Header: e:\\netlabs.cvs\\ext2fs/src/vfs/dcache.c,v 1.1 2001/05/09 17:48:06 umoeller Exp $
//
...
INLINE void remove_lru(struct dir_cache_entry * de)
{
        de->next_lru->prev_lru = de->prev_lru;
        de->prev_lru->next_lru = de->next_lru;
}

INLINE void add_lru(struct dir_cache_entry * de, struct dir_cache_entry *head)
{
        de->next_lru = head;
...
INLINE void update_lru(struct dir_cache_entry * de)
{
        if (de == *de->lru_head)
 ...
 */
INLINE unsigned long namehash(const char * name, int len)
{
 ...
 */
INLINE void remove_hash(struct dir_cache_entry * de)
{
        if (de->h.next) {
...
INLINE void add_hash(struct dir_cache_entry * de, struct hash_list * hash)
{
        de->h.next = hash->next;
 ...
 */
INLINE void move_to_level2(struct dir_cache_entry * old_de, struct hash_list * hash)
{

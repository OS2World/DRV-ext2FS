//
// $Header: e:\\netlabs.cvs\\ext2fs/src/ext2/dir.c,v 1.1 2001/05/09 17:53:01 umoeller Exp $
//
                         ...
                         struct dirent * dirent, int count)
{
        ...
        struct buffer_head * bh, * tmp, * __bha[16];
    struct buffer_head **bha = __StackToFlat(__bha);
        ...
        while (count > 0 && !stored && filp->f_pos < inode->i_size) {
                ...
                bh = ext2_bread (inode, blk, 0, __StackToFlat(&err));
                if (!bh) {
                ...
                if (!offset) {
                        for (i = 16 >> (EXT2_BLOCK_SIZE_BITS(sb) - 9), num = 0;
                             i > 0; i--) {
                                tmp = ext2_getblk (inode, ++blk, 0, __StackToFlat(&err));

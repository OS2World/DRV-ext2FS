//
// $Header: e:\\netlabs.cvs\\ext2fs/src/ext2/ialloc.c,v 1.1 2001/05/09 17:53:02 umoeller Exp $
//
...
#else
        if (!clear_bit (bit, (PINT32)bh->b_data))
#endif
...
//#else
//        inode->i_flags = (unsigned short)sb->s_flags;
//#endif
        lock_super (sb);
        es = sb->u.ext2_sb.s_es;
repeat:
...
/* I am not yet convinced that this next bit is necessary.
                ...
                for (j = 0; j < sb->u.ext2_sb.s_groups_count; j++) {
                        tmp = get_group_desc (sb, i, __StackToFlat(&bh2));
...
*/
                if (!gdp) {
                        for (j = 0; j < sb->u.ext2_sb.s_groups_count; j++) {
                                tmp = get_group_desc (sb, j, __StackToFlat(&bh2));

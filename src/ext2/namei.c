//
// $Header: e:\\netlabs.cvs\\ext2fs/src/ext2/namei.c,v 1.1 2001/05/09 17:53:02 umoeller Exp $
//
...
unsigned long event = 0;    // To be moved somewhere else
unsigned short umask = 007; // To be moved somewhere else
#else // OS2
                                             ...
                                             struct ext2_dir_entry ** res_dir)
{
        ...
        struct buffer_head * __bh_use[NAMEI_RA_SIZE];
        struct buffer_head * __bh_read[NAMEI_RA_SIZE];
        struct buffer_head **bh_use  = __StackToFlat(__bh_use);
        struct buffer_head **bh_read = __StackToFlat(__bh_read);
        ...
        *res_dir = NULL;
        ...
        memset (bh_use, 0, sizeof (__bh_use));
        toread = 0;
        for (block = 0; block < NAMEI_RA_SIZE; ++block) {
                ...
                if ((block << EXT2_BLOCK_SIZE_BITS (sb)) >= dir->i_size)
                        break;
                bh = ext2_getblk (dir, block, 0, __StackToFlat(&err));
#ifdef OS2
                if (!bh) {
                    kernel_printf("ext2_find_entry - ext2_getblk = NULL - block = %lu", block);
                }
#endif
                bh_use[block] = bh;
                if (bh && !bh->b_uptodate)
        ...
        while (offset < dir->i_size) {
                ...
                if (((block + NAMEI_RA_SIZE) << EXT2_BLOCK_SIZE_BITS (sb)) >=
                    dir->i_size)
                        ...
                        bh = ext2_getblk (dir, block + NAMEI_RA_SIZE, 0, __StackToFlat(&err));
                 ...
                 struct inode ** result)
{
        ...
        *result = NULL;
        ...
        if (dcache_lookup(dir, name, len, __StackToFlat(&ino))){
                if (!ino) {
                        iput(dir);
                        return -ENOENT;
                }
        ...
        ino = dir->i_version;
        if (!(bh = ext2_find_entry (dir, name, len, __StackToFlat(&de)))) {
        ...
        while (1) {
                ...
                if ((de->inode == 0 && de->rec_len >= rec_len) ||
                    (de->rec_len >= EXT2_DIR_REC_LEN(de->name_len) + rec_len)) {
                        offset += (unsigned long)de->rec_len;                // Useful ????
                        if (de->inode) {
                 ...
                 struct inode ** result)
{
        ...
        *result = NULL;
        ...
        inode->i_mode = mode;
        inode->i_dirt = 1;
        bh = ext2_add_entry (dir, name, len, __StackToFlat(&de), __StackToFlat(&err));
        if (!bh) {
                inode->i_nlink--;
                inode->i_dirt = 1;
                iput (inode);
                ...
                int rdev)
{
        ...
        if (!dir)
                return -ENOENT;
        bh = ext2_find_entry (dir, name, len, __StackToFlat(&de));
        if (bh) {
                brelse (bh);
                iput (dir);
                return -EEXIST;
...
#ifndef OS2                                     // no relevant uid on OS/2
...
int ext2_mkdir (struct inode * dir, const char * name, int len, int mode)
{
        ...
        dir_block = ext2_bread (inode, 0, 1, __StackToFlat(&err));
        if (!dir_block) {
                iput (dir);
                inode->i_nlink--;
                inode->i_dirt = 1;
...
#else
        inode->i_mode = S_IFDIR | (mode & S_IRWXUGO & ~umask);
#endif
        if (dir->i_mode & S_ISGID)
                inode->i_mode |= S_ISGID;
        inode->i_dirt = 1;
...
static int empty_dir (struct inode * inode)
{
        ...
        if (inode->i_size < EXT2_DIR_REC_LEN(1) + EXT2_DIR_REC_LEN(2) ||
            !(bh = ext2_bread (inode, 0, 0, __StackToFlat(&err)))) {
        ...
        while (offset < inode->i_size ) {
                if ((void *) de >= (void *) (bh->b_data + sb->s_blocksize)) {
                        brelse (bh);
                        bh = ext2_bread (inode, offset >> EXT2_BLOCK_SIZE_BITS(sb), 1, __StackToFlat(&err));
                        if (!bh) {
                  ...
                  const char * symname)
{
        ...
        if (l >= EXT2_N_BLOCKS * sizeof (unsigned long)) {
                ...
                name_block = ext2_bread (inode, 0, 1, __StackToFlat(&err));
                if (!name_block) {
                        iput (dir);
                        inode->i_nlink--;
                        inode->i_dirt = 1;
...
static int subdir (struct inode * new_inode, struct inode * old_inode)
{
        ...
        for (;;) {
                ...
                if (new_inode->i_dev != old_inode->i_dev)
                        break;
                ino = new_inode->i_ino;
                if (ext2_lookup (new_inode, "..", 2, __StackToFlat(&new_inode)))
                        break;
...
start_up:
        ...
        new_de = NULL;
        old_bh = ext2_find_entry (old_dir, old_name, old_len, __StackToFlat(&old_de));
        retval = -ENOENT;
        if (!old_bh)
                goto end_rename;
        ...
        if (IS_APPEND(old_inode) || IS_IMMUTABLE(old_inode))
                goto end_rename;
        new_bh = ext2_find_entry (new_dir, new_name, new_len, __StackToFlat(&new_de));
        if (new_bh) {
        ...
        if (S_ISDIR(old_inode->i_mode)) {
                ...
                if (subdir (new_dir, old_inode))
                        goto end_rename;
                dir_bh = ext2_bread (old_inode, 0, 0, __StackToFlat(&retval));
                if (!dir_bh)
                        goto end_rename;
        ...
        if (!new_bh)
                new_bh = ext2_add_entry (new_dir, new_name, new_len, __StackToFlat(&new_de),
                                         __StackToFlat(&retval));
        if (!new_bh)
                goto end_rename;
        new_dir->i_version = ++event;
        /*
        ...
        if (IS_SYNC(old_dir)) {
                ll_rw_block (WRITE, 1, __StackToFlat(&old_bh));
                wait_on_buffer (old_bh);
        }
        mark_buffer_dirty(new_bh, 1);
        if (IS_SYNC(new_dir)) {
                ll_rw_block (WRITE, 1, __StackToFlat(&new_bh));

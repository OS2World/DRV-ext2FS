//
// $Header: e:\\netlabs.cvs\\ext2fs/src/ext2/file.c,v 1.1 2001/05/09 17:53:01 umoeller Exp $
//
                    ...
                    char * buf, int count)
{
        ...
        struct buffer_head * __bhreq[NBUF];
        struct buffer_head * __buflist[NBUF];
        struct buffer_head **bhreq   = __StackToFlat(__bhreq);
        struct buffer_head **buflist = __StackToFlat(__buflist);
...
#else
                        *bhb = ext2_getblk (inode, block++, 0, __StackToFlat(&err));
#endif
                            ...
                            char * buf, int count)
{
        ...
        off_t pos2;
        int written, c;
        struct buffer_head * bh, *__bufferlist[NBUF];
        struct buffer_head **bufferlist = __StackToFlat(__bufferlist);
        char * p;
        struct super_block * sb;
        int err;
        ...
        while (written < count) {
                ...
                bh = ext2_getblk (inode, pos2 / sb->s_blocksize, 1, __StackToFlat(&err));
                if (!bh) {
                        if (!written)
                                written = err;
                        break;
